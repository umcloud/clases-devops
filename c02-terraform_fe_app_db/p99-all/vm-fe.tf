data "template_file" "my_fe_init" {
  template = file("${path.module}/templates/vm-fe.init.sh")
  vars = {
    app_ip = openstack_compute_instance_v2.my_app.network.0.fixed_ip_v4
  }
}
resource "openstack_compute_instance_v2" "my_fe" {
  ...
  security_groups   = ["sg_fe"]
  availability_zone = "nodos-amd-2022"

  user_data = data.template_file.my_fe_init.rendered
